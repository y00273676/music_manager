(function($){
	$.fn.uploadBtnMD5=function(url,successFn,beforFn){
		var input=null;
		var div=null;
		var ul=null;
		...
		function eachFile(i){
			if (input.files.hasOwnProperty(i)&&!isNaN(i)&&!!input.files[i]){
				...
				var name=file.name;
				var initName=name;
				var nameEnd=name.lastIndexOf(".")==-1?"":name.substring(name.indexOf("."),name.length);
				var size=file.size;
				...
				reader.readAsDataURL(smallData);
			}
		}
		function sendAjax( url, file, name, start, end, initName, path, liDivDiv, span){
			...
			$.ajax({
				...
				success:function(str){
					...
					start += postSize;
					if(end<size){
						showSuccess( end, size, name, path, liDivDiv, span);
