(function($){
	$.fn.uploadBtn=function(url,successFn,beforFn){
		var input=null;
		var div=null;
		var ul=null;
		...
		function eachFile(i){
			if (input.files.hasOwnProperty(i)&&!isNaN(i)&&!!input.files[i]){
				...
				if(!!beforFn){
					beforFn(file,function(no){
						name=no+"."+nameEnd;
						var start = 0;
						var end=Math.min(size,start+postSize);
						sendAjax(url, file, name, start, end, initName, "", liDivDiv, span, no);
						i++;
						eachFile(i);
					});
				}else{
					var start = 0;
					var end=Math.min(size,start+postSize);
					sendAjax(url, file, name, start, end, initName, "", liDivDiv, span, "0");
					i++;
					eachFile(i);
				}
		...
		function sendAjax( url, file, name, start, end, initName, path, liDivDiv, span, no){
			var size=file.size;
			var postSize=2*1024*1024;
			...
			$.ajax({
				...
				success:function(str){
					...
					start += postSize;
					if(end<size){
						showSuccess( end, size, initName, path, liDivDiv, span, no);
						end = Math.min(size, start+postSize);
						if(!!str.data&&str.data!="0"&&parseInt(str.data)>end){
							start=parseInt(str.data);
							end = Math.min(size, start+postSize);
						}
						sendAjax(url, file, name, start, end, initName, path, liDivDiv, span, no);
					}else{
						showSuccess( end, size, initName, path, liDivDiv, span, no);
					}
				},
		...
		function showSuccess( end, size, initName, path, liDivDiv, span, no){
			var num=parseInt(end/size*100);
			liDivDiv.css("width",num+"%");
			span.html(num+"%");
			if(num==100){
				if(!!successFn)
					successFn(initName,path,size,no);
				span.html("上传完成!");
			}
		}
	}
